...
def analyse():
    ...
    ax3.set_xlabel('Mois'); ax3.set_ylabel('Nombre')
...
def export_csv():
    ...
    si = StringIO()
    cw = csv.writer(si)
    cw.writerow(['ID', 'Quartier', 'Type', 'Description', 'Statut', 'Date', 'Email anonyme', 'Image'])
    for s in signalements:
        cw.writerow([
            s.id,
            s.quartier,
            ...
            s.description or '',
            s.statut,
            s.date_signalement.strftime('%Y-%m-%d'),
            s.email_anonyme or '',
            s.image_filename or ''
        ])
    ...
    return send_file(
        ...
        mimetype='text/csv',
        as_attachment=True,
        download_name='signalements.csv'
    )
